package com.sap.netweaver.cloud.sample.integrationtest;
 ...
 */
public class PersonsJpaPersistenceIntegrationTest {
    private static String serverUrl;
    private static String nameSupplement;
    private static String webModuleContextRoot;
    ...
    public static void setupSuite() {
        webModuleContextRoot = "e2e-nwcloud-app-persons-list-web-orig";
        servletName = "PersonsJpaPersistenceServlet";
    ...
    /**
     * Call the main servletName and check that it contains the expected content.
     */
    @Test
    public void testPersistenceWithJPAServlet() throws IOException, SAXException {
        // Call servlet
        request = new GetMethodWebRequest(serverUrl + "/" + webModuleContextRoot + "/" + servletName);
        response = wc.getResponse(request);
        Assert.assertThat(response.getText(), containsString("Servlet for Adding and Displaying persisted Person Entities (with JPA)"));
    }
     ...
     */
    @Test
    public void testAddingFirstPerson() throws IOException, SAXException {
        // Call servlet
        request = new GetMethodWebRequest(serverUrl + "/" + webModuleContextRoot + "/" + servletName);
        response = wc.getResponse(request);
    ...
    /**
     * Try to add another person and check that this and the former person are found.
     */
    @Test
    public void testAddingSecondPerson() throws IOException, SAXException {
        // Call servlet
        request = new GetMethodWebRequest(serverUrl + "/" + webModuleContextRoot + "/" + servletName);
        response = wc.getResponse(request);

        // Fill out form and submit another person
        WebForm form = response.getForms()[0];
        form.setParameter("FirstName", "Jane_" + nameSupplement);
        form.setParameter("LastName", "Doe_" + nameSupplement);
        response = form.submit();
        Assert.assertThat(response.getText(), containsString("John_" + nameSupplement));
        Assert.assertThat(response.getText(), containsString("Jane_" + nameSupplement));
        Assert.assertThat(response.getText(), containsString("Doe_" + nameSupplement));
    }
}